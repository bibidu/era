...
**风水风格特例**：若用户提到「风水」「风水风格」「风水质感」等，**直接**使用风水模板（`pageOverlay: 'fengshui'`），并**固定抖音尺寸**（`aspectRatio: '9:16'`），同时 **关闭「全文 xxx 字」**（`showWordCount: false`），**顶部文案固定为**：

`连续观看、点赞、关注，你也是地理风水达人（阳宅篇）`

（写入 `topText`；无需再问导出平台，除非用户另指定小红书。）
...
5. 确认后：`era_create_project` / `era_set_markdown` 写入工程（默认 `pageOverlay: 'pixel'`；若用户已指定风水风格则用 `pageOverlay: 'fengshui'` + `aspectRatio: '9:16'` + `showWordCount: false` + `topText: '连续观看、点赞、关注，你也是地理风水达人（阳宅篇）'`）。
...
- **一页最多 3 处计入密度的高亮**（一处 = 一个连续高亮片段/range；含 brush/underline/circle 任一）。**列表 li 子标题整组高亮不计入**（见下条）。极个别页面信息密度极高时才可放宽，并在说明里写明原因。
- **必须结合段落语义**选词：只标真正改变理解的关键词/结论句，禁止机械均匀撒点；**禁止**只标收尾几个字充数（例如总结段不应只高亮「伪装成智能」这类非核心尾巴，应标决策系统能力或核心判断）。段落重点宜多用 **笔刷（brush）**。
- **二级标题（`##` / heading）不要任何高亮**。正文层级优先只用 `#`（一级）与 `##`（二级），避免再用 `###` 及以下。
- **列表 `ul/li` 子标题连同规则**（重要）：
  - 同一小节下连续的「子标题：说明」列表，若要对子标题高亮，必须**整组同开同关**：要高亮则每个 li 冒号前的子标题都高亮，且**样式与颜色完全一致**；不高亮则整组都不高亮。
  - 例：`宜亮` / `宜静、宜整洁` / `宜有靠山` —— 要么全用同一刷子（或全用同一下划线），要么全不用。
  - **只高亮冒号前的子标题**，冒号后的说明一律不高亮；禁止整段 li 全刷。
  - 列表子标题高亮优先用 **笔刷（brush）**；若用下划线，整组也必须同为下划线。
  - **计数例外**：上述整组 li 子标题高亮**不计入**「一页最多 3 处」密度上限（整组视为样式连同，不占名额）。
- **一句话或两句话的重点**：可用 **下划线**；若当前页计入密度的高亮未近上限，可 **下划线 + 笔刷叠加**（叠加计为 2 处，慎用；**两种样式必须同色**，见 §4.2）。
- **一级标题**：默认至少一处 **画圈（circle）**；禁止对标题用 `quote`。若用户明确禁止画圈（或风水等场景改用首句着色），可用 `titlePrimaryColor` 代替画圈，此时不再强制 circle。
...
1. `era_update_config` 设 `aspectRatio`（保持当前模板：默认 `pageOverlay: 'pixel'`，风水风格用 `fengshui` 且 `showWordCount: false` + 固定 `topText`；`titleLineHeight` 不过松）
2. `era_preview_layout`
3. 若有告警必须先修再导出，包括：
   - 单行溢出、孤行、独行标点
   - **标题缺少画圈**（已设 `titlePrimaryColor` 时可豁免）
   - **画圈词语跨行**
   - **高亮颜色超过 3 种**
   - **一页计入密度的高亮超过 3 处**（li 子标题整组除外）
   - **标题行高过松**
...
| 画幅/模板 | `era_update_config` · `PATCH .../config`（优先 `pageOverlay: 'pixel'`；风水风格用 `fengshui` + `9:16` + `showWordCount: false` + 固定 `topText`） |
| 顶部文案 | `era_update_config` · `PATCH .../config`（普通模板可自定义 `topText`；风水模板必须固定为「连续观看、点赞、关注，你也是地理风水达人（阳宅篇）」；`showWordCount: false` 隐藏「全文 xxx 字」） |
