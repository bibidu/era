...
| 吸粉率 | 末页给不给关注理由 | 0.03%–0.32%，偏低 | 末页必须关注理由 + 提问（不写期数、不写下期预告；期数只在顶栏） |
...
脚本自动给出：**本期期号**、**建议档期**、按时段/星期的播放中位数、Top3 / Bottom3、受众众数。
（若仍检出历史篇末 `carryOverPromise`，仅作选题参考，不强制接住；本期也不再新写「下期预告」。）
...
1. **AI 实践与技巧**优先：怎么跟 Agent 协作、工作流取舍、判断力、能立刻改进 AI 编码实践的 prompt / 规则。
   **不要**以裸 git / CLI 命令教程当钩子或正文主体；git / CLI 只可作服务上述实践点的小辅助细节。
2. **旧篇重做（第 26 期起优先烧队列）**：优先把 **2026-07-31 之前、播放量低于 500** 的图文（提取成功）重新换个措辞发一下。内容点可复用，**禁止原句 / 原标题重发**。换标题必须仍过标题公式（我 + 数字/具体 + 反预期；禁用秘诀 / 教你 / 纯英文）。已按新标题重做过的不要再做；**不要重做 07-31 及之后**（3 行规则 633、AGENTS.md 11000 等）。队列见 playbook §旧篇重做；**队列清空后回落到下面 3–5**。
3. 高播放作品的**相邻话题**，但不连发同一名词超过 3 期（AGENTS.md 已连发 3 期即为疲劳信号）。
4. 受众关注的同类作者在做、而本账号还没做过的实操点。
5. 复用曾因**档期或标题**失手、内容本身不差的选题（改标题重做，不要浪费）。
...
- 必须有一页是**能直接抄走的东西**（规则原文 / prompt / 判断清单；命令仅当服务 AI 实践点时附带）——本账号收藏常大于点赞。
  判断标准：读者只截这一页、不看正文，能不能直接用起来。可抄页落地句写成「可自行封装 skill，或写进 AGENTS.md。」不要「按仓库来」，不要「交给子 Agent」，不要「复制粘贴一下 / 直接粘给 Agent」。
  **「我的目录长这样」「结构示意」不算**——第 16 期给目录结构，浏览图片数达标但吸粉/收藏同量级最低（见 playbook §版面）
- 术语首现必须一句话解释，不假设读者用过（受众里在校大学生占 32%–52%）
- Agent 量词用「个」不用「只」：写代码那个 / 换一个 / 第二个 / 开两个。**「只审」保留**（=只负责审，不是量词）
- **末页两件套**：关注理由（一句身份认同）+ 一个提问。期数只走顶栏 `topText`，**不要**再写一遍 `每天一个提效实操·第 N 期`。
  **禁止**写「下期：…」预告
- 封面除大标题外的英文、以及正文和二级标题里的英文，用 IBM Plex Mono；中文仍用宋体 / 数黑体；代码块字体对齐 GitHub 网页端栈；数字 token 用 Liberation Mono，中文注释同一栈回落 Noto Sans CJK。代码块里中文行必须以 `// ` 开头（渲染会自动补）；按 JavaScript 高亮，注释/数字/关键字/字符串/标识符不同色。
- **所有内容页顶栏用 `topText`**：`topText: '每天一个提效实操·第 N 期'`，`seriesLabel` 留空（不要朱红下划线、不要额外空行）。末页正文**不要**再写同一句。
...
7. 有一页能直接抄走；术语首现有解释
8. 末页＝关注理由 + 提问（无期数、无下期预告）；所有内容页顶栏是 `每天一个提效实操·第 N 期`（`topText`，`seriesLabel` 空，无朱红）；选题落在 AI 实践与技巧，不是裸 git/CLI 教程
