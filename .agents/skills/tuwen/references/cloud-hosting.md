...
- 脚本：
  ...
  - 仅清理：`bash scripts/oss-cleanup-expired.sh`（或 `--dry-run`）
...
1. 导出/渲染后必须上传 OSS，再把**签名 URL 直接发在对话框**（不要发裸对象 URL / 本地路径 / 用 HTML 嵌入代替）。
2. HTML 多图交付：只替换 `src` / `url(...)` 为签名 URL；确认与发图仍以对话框直发为准。
3. 签名过期后可用 `--sign` 重新生成；不要把对象改成 public-read。
4. 不要绕过 `oss-upload.sh` 直接 `ossutil cp`，否则会漏掉 14 小时过期清理。

### 过期清理（省存储费）

签名 12h 过期后对象仍计费。每次 `oss-upload.sh` 存图前会删除 `era/assets/` 下 LastModified **超过 14 小时**的对象。详见 `docs/cloud-hosting.md`。
