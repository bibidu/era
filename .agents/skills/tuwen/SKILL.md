...
1. **封面标题（即社媒标题）**：直接问用户 **封面标题第一行**、**第二行**（缺哪行问哪行；用户已给出则不必再问）。两行连在一起（无额外分隔）即为社媒 `title` / 入库标题；封面 `bigTitle` 按两行排版。**不要**再给 5 个标题候选。精细排版可发标题设置页 `https://39.106.179.17.sslip.io/?tab=title&text=<两行连写标题>`（多行可用 `%0A`；禁止裸 `?tab=title`）。
2. **封面字段**：从内容提取并确认——见 §封面字段表（小标题、描述、标签、二级标题、主题色等；大标题已由上一步两行确定）。
...
| 字段 | 必填 | 说明 |
| --- | --- | --- |
| `bigTitle` | ✅ | 大标题两行（`\\n` 或数组）；= 封面标题第一行 + 第二行；两行连写即社媒标题；英文建议全大写 |
