...
**系列期数顶栏（整套第 2 页＝内容首页）**：算上封面时的**第 2 页**必须在正文区顶栏展示 `seriesLabel`（如 `每天一个提效实操·第15期`），**禁止**省略或只写在对话里。该文案下带**朱红下划线** `#C41E3A`（**只划到文字结束，不拉满整行**）；红线与下方第一个二级标题（`##`）之间空约 **三行正文**（按正文 `bodyFontSize × bodyLineHeight × 3` 的页像素间距，见 `SERIES_LABEL_GAP_LINES`）。期数按连载递增；用户未指定时先问当期期号。其余内容页顶栏仍为 `点赞关注不迷路～`（无期数红线）。
...
1. **封面标题（即社媒标题）**：直接问用户 **封面标题各行**（常见第一行、第二行；可三行及以上；缺哪行问哪行；用户已给出则不必再问）。各行按顺序连在一起（无额外分隔）即为社媒 `title` / 入库标题；封面 `bigTitle` 按多行排版，默认数黑体。**不要**再给 5 个标题候选。精细排版可发标题设置页 `https://39.106.179.17.sslip.io/?tab=title&text=<各行连写标题>`（多行可用 `%0A`；禁止裸 `?tab=title`）。
2. **封面字段**：从内容提取并确认——见 §封面字段表（小标题、描述、标签、二级标题、主题色、`blobCorner`、按行配色等；大标题已由上一步各行确定）。
...
- 建工程：`pageOverlay: 'pixel'`、`9:16`（或用户指定 `3:4`）、`headingFontId: shuheiti`、`topText: '点赞关注不迷路～'`、`showWordCount: false`、**必须** `seriesLabel: '每天一个提效实操·第N期'`（N 为当期期号；内容首页顶栏展示 + 朱红下划线 + 距下方 `##` 空三行）。
- 导出前确认：整套第 2 页顶栏＝`seriesLabel`＋朱红下划线（贴字宽）＋与 `##` 空三行；后续页顶栏＝点赞关注；每页以一个 `##` 起头。
