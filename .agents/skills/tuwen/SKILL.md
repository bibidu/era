...
**系列期数顶栏（整套第 2 页）**：算上封面时的第 2 页＝内容首页。必须设置 `seriesLabel`（如 `每天一个提效实操·第14期`）：该页顶栏显示此文案、**底边朱红线** `#C41E3A`（**只划到文字结束，不拉满整行**），与下方二级标题间距约 **三行**正文。期数按连载递增；用户未指定时先问当期期号。其余内容页顶栏仍为 `点赞关注不迷路～`。
