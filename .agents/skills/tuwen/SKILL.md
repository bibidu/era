...
**系列期数放末页（硬性）**：`每天一个提效实操·第 N 期` 与「下期预告」写在**最后一页正文的结尾两行**，`seriesLabel` 配置留空（这样每页顶栏都是 `点赞关注不迷路～`）。
...
**末页三件套**：连载承诺（下期具体讲什么，不能写「敬请期待」）+ 关注理由（一句身份认同）+ 一个提问。吸粉率的天花板在这一页。

**翻页钩子**：每页结尾留一句悬念或提问，否则读者停在第 2 页。

**可抄页**：每篇至少一页是能直接抄走的东西（命令原文 / 规则原文 / 判断清单）。本账号爆款的收藏量常大于点赞量。判断标准是**只截这一页就能用起来**；「我的目录长这样」这类结构示意不算——第 16 期这么做，浏览图片数达标但吸粉率与收藏率都是同播放量级最低（见 [playbook](../shedashi/references/playbook.md) §版面）。
...
- **末页正文结尾两行**固定为 `每天一个提效实操·第 N 期` 与 `下期：<下期具体讲什么>`；其上是关注理由 + 一个提问。
- 导出前自查：每页顶栏都是 `点赞关注不迷路～`；每页以一个 `##` 起头；每页末尾有翻页钩子；至少一页可直接抄走；末页有三件套 + 期数。
