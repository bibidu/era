...
**凡交付给用户的图片（拼图总览、封面、各内容页），都必须：**

1. 先上传阿里云 OSS（私有读）
2. 在**对话框里直接发送**该图的 **12 小时签名 URL**（纯链接文本即可）
3. **禁止**用 HTML 页面嵌入图片链接代替对话框发图
4. **禁止**只发本地路径、未签名裸 OSS URL、或「请打开某个 HTML 查看」

用户必须能在对话里点开链接看到图；否则视为未交付。
...
1. 导出后**先只发拼图**（横版总览）给用户：上传 OSS 后在对话框**直接发送签名 URL**；非风水流程的拼图须为 **封面图 + 各内容页** 的拼合版。提醒可放大逐页查看，此步不附各页独立图。
2. **明确询问**：拼图效果是否 OK？要改高亮/内容/封面/画幅吗？
3. 用户要改 → 修改 → 重新校验/导出/拼图 → 仍先只发拼图签名 URL 再问。
4. **仅当用户明确确认后**，再**逐张单独**发送封面图与各页独立 PNG：每张先 `bash scripts/oss-upload.sh`，再在对话框发签名 URL。
5. 若用户同时要小红书与抖音：每个比例各自「先拼图确认 → 再分图」，不混发。
