...
2. **主题色**：只用于徽章底、强调短横线、细网格/技术线、大几何色块、页脚图标
3. **大标题**：超粗、高压缩无衬线（Anton / Impact 系），全大写，占画面上半区视觉重量；颜色用 `bigTitleColor`
4. **信息区**：主题色短横线 → 粗体小标题 → 常规描述 → 浅底圆角标签条（`·` 分隔）
5. **页脚**：二级标题横排，前缀线稿图标，竖线分隔
6. **装饰**：淡主题色网格/十字分割线、角落点阵、右下大圆色块出血、细同心圆弧
7. **留白**：中部保持呼吸感，不要塞满文案或装饰
