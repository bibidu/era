...
2. **核心区（重要）**：除背景色与正方形网格外，**所有内容**落在上下居中、**左右各留 50px** 的 **3:4** 区域（`980×1307`，左右内缩增强中心感）。因 9:16 社媒个人主页预览裁的是画面中心 3:4。正方形网格铺满整张 9:16。可用 `node scripts/generate-cover-layout-review.mjs <cover.png>` 生成标注对照图与 3:4 裁切预览。
