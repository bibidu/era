...
name: fengshui
description: >-
  【风水竖版成片】用户给抖音/视频链接时使用：抽中文口播、改词<5%、山水静图、片头2秒毛笔标题+锦垣印、宋体100字幕、拼 9:16 成片。
...
- **只**丢 OSS 裸链当唯一交付
- 改动用户 Mac；底音不在用户 Mac 上——配音在**云机**完成
...
- 云机已部署 **VoxCPM 0.5**。
- **克隆「老者」**：原 fengshui-B / MiniMax B 轨转成的 prompt（固定资产；在云机侧使用，**不要**回落到 MiniMax）。
- **禁止 MiniMax**。
- 合成前先通篇听参考/底音有无杂音，再整篇合成。
- 底音与合成都在云机，不在用户 Mac。
...
2. 配音＝云机 VoxCPM 0.5 + 老者；无 MiniMax
